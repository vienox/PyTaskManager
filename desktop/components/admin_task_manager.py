...
def create_admin_task_manager(page: ft.Page, api):
    ...
    Args:
        ...
        api: APIClient instance for backend communication
        
    Returns:
        tuple: (widget, load_tasks_function) - Main UI widget and refresh function
    """
    ...
    def show_error(error_text_control, exception):
        """
        Display user-friendly error message based on exception.
        
        Args:
            error_text_control: ft.Text control to display error
            exception: Exception object or error message string
        """
        error_msg = str(exception)
        
        if "401" in error_msg or "403" in error_msg:
            error_text_control.value = "Insufficient permissions. Please log in again as admin."
        elif "404" in error_msg:
            error_text_control.value = "Resource does not exist"
        elif "500" in error_msg:
            error_text_control.value = "Server error. Please try again."
        else:
            error_text_control.value = f"Error: {error_msg}"
